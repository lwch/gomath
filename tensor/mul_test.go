...
func mulScalar(s float32, values []float32) []float32 {
...
func mulVector(x, w []float32) []float32 {
...
func testMul(t *testing.T,
	...
	values := mulScalar(nScalar, vectorValues)
	...
	for i := int64(0); i < rows; i++ {
		tmp := mulVector(vectorValues, fp32Matrix.Storage().Row(i).([]float32))
	...
	values = make([]float32, rows*cols)
	for i := int64(0); i < rows; i++ {
		tmp := mulVector(fp32Matrix.Storage().Row(i).([]float32), vectorValues)
		copy(values[i*cols:(i+1)*cols], tmp)
	}
	compareAndAssert(t, matrix.Mul(vector).Storage(), values, "matrix * vector")

	// matrix * matrix
	values = mulVector(matrixValues, matrixValues)
